    ...
    while True:
        ...
        if from_client[0] == 'read':
            ...
            if len(from_client) !=2:
                ...
                reply = reply + "Error : Invalid command. Please enter filename"
                ...
                if filename in files:
                    ...
                    if filename in currently_writing:
                        ...
                        reply = reply + "Access denied. " + filename + " currently being written" + "\nWould you like to wait or continue with other operation? : "
                        connection.send(str.encode(reply))
                        data1 = connection.recv(2048)
                        from_client = data1.decode('utf-8')
                        if from_client != "wait":
                            connection.send(str.encode('Choosing to abort'))
                        else:
                            connection.send(str.encode('...waiting...'))
                            while (filename in currently_writing):
                                pass
                            connection.send(str.encode('Other client reliquinshed write access'))
                    ...
                    # check if file is being read already
                        ...
                        if client_id in clients and clients[client_id] == filename:
                            ...
                            reply = reply + "You already have the read access"
                        else: 
                            clients[client_id] = filename
                            print("Client ",client_id,' has been granted a Read access')
                        reply = reply+"\nRead access only!!\nContents of "+filename+" are : \n***************************************************************\n"+files[filename]+"\n***************************************************************\n"
                        ...
                        clients[client_id] = filename
                        print("Client ",client_id,' has been granted a Read access')
                        reply = reply+"Contents of "+filename+" are : \n***************************************************************\n"+files[filename]+"\n***************************************************************\n"
        ...
        # stop reading
            ...
            if len(from_client) !=2:
                ...
                if client_id in clients and clients[client_id] == filename:
                    ...
                    if filename in files:
                        filename = from_client[1]
                        # file is not present in currently_reading then incorrect filename is provided. send error
